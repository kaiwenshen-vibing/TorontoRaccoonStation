...
from app.schemas.booking import CreateCharacterClientMatchRequest, UpdateCharacterClientMatchRequest
...
async def test_create_match_requires_incomplete():
    ...
    service = CharacterClientMatchService(session=session)

    with pytest.raises(ConflictError):
        await service.create_match(
            store_id=1,
            booking_id=10,
            payload=CreateCharacterClientMatchRequest(character_id=1, client_id=1),
        )
...
async def test_update_match_not_found():
    ...
    with pytest.raises(NotFoundError):
        await service.update_match(
            ...
            booking_id=10,
            match_id=5,
            payload=UpdateCharacterClientMatchRequest(character_id=2),
        )
